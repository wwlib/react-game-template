...
export interface LanderGameCanvasProps {
  shipCoords: any;
  shipThrust: any;
  shipFuel: number;
...
export interface LanderGameCanvasState {
  shipCoords: any;
  shipThrust: any;
  shipFuel: number;
  gameStatus: GameStatus;
...
export default class LanderGameCanvas extends React.Component<LanderGameCanvasProps, LanderGameCanvasState> {

  public log: Log;

  constructor(props: LanderGameCanvasProps) {
    super(props);
    this.state = {
  ...
  componentWillReceiveProps(nextProps: LanderGameCanvasProps) {
    // console.log(nextProps);
    if (nextProps.shipCoords !== this.state.shipCoords || nextProps.shipThrust !== this.state.shipThrust || nextProps.gameStatus !== this.state.gameStatus || nextProps.shipFuel !== this.state.shipFuel) {
      this.setState({
  ...
  render() {
    ...
    return (
      <div className='LanderGameCanvas'>
        <svg viewBox={`0 0 ${width} ${height}`} style={{ border: '1px solid black' }}>
          ...
          <svg viewBox={`0 0 80 80`} x={ship.x} y={ship.y} width={80} height={80}>
            <rect stroke={stroke} fill={fill} x={20} y={20} width={40} height={40} strokeWidth="3%" />
